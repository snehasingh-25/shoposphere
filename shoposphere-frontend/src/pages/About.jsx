...
export default function About() {
  const [aboutReels, setAboutReels] = useState([]);
  const [reelsReady, setReelsReady] = useState(false);

  useEffect(() => {
    const ac = new AbortController();
    fetch(`${API}/reels?placement=about`, { signal: ac.signal })
      .then((res) => res.json())
      .then((data) => setAboutReels(Array.isArray(data) ? data : []))
      .catch(() => setAboutReels([]))
      .finally(() => setReelsReady(true));
    return () => ac.abort();
  }, []);
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "var(--page-products-bg)" }}>
      <main className="pt-10 sm:pt-14 pb-24">
        ...
        {/* Our Story in Motion — reels managed in Admin → Reels → About page reels */}
        {reelsReady && aboutReels.length > 0 ? (
          <section className="mb-20 sm:mb-32">
            ...
            <div className="flex overflow-x-auto scrollbar-hide gap-5 sm:gap-2 px-6 md:px-24 pb-4 snap-x snap-mandatory">
              {aboutReels.map((r) => {
                const href = r.videoUrl || r.url;
                const label = r.title || `Reel ${r.id}`;
                const inner = (
                  <>
                    <img
                      alt={label}
                      className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"
                      src={r.thumbnail || "/logo.png"}
                      loading="lazy"
                    />
                    ...
                    <div className="absolute inset-0 bg-linear-to-t from-black/60 to-transparent flex items-end p-5 sm:p-6">
                      <div className="text-white">
                        {r.caption ? (
                          <p className="text-[10px] font-semibold uppercase tracking-widest mb-1 opacity-80">{r.caption}</p>
                        ) : null}
                ...
                const cardClass =
                  "flex-none w-64 sm:w-72 aspect-9/16 rounded-2xl overflow-hidden relative snap-start group block focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[var(--foreground)]";
